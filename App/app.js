(() => {
  ...
  const EXTRA_I18N = {
    en: { clipboardEmpty: "Clipboard text is empty. If you copied a link from another app, paste it into the manual input box and convert.", clipboardImageOnly: "Clipboard contains image data, not link text. Copy a link or paste text manually.", clipboardNoReadableText: "Clipboard has no readable text URL. Paste the link into the manual input box and convert.", clipboardUnavailable: "Clipboard reading is not available in this browser/context. Paste manually instead.", pixivCopyBlocked: "Converted, but the browser blocked automatic copy. Tap “Copy result”." },
    zh: { clipboardEmpty: "剪贴板文本为空。如果你是从其他 App 复制链接，请粘贴到手动输入框后转换。", clipboardImageOnly: "剪贴板里是图片数据，不是链接文本。请复制链接或手动粘贴文字。", clipboardNoReadableText: "剪贴板里没有可读取的文本链接。请粘贴到手动输入框后转换。", clipboardUnavailable: "当前浏览器/环境不可读取剪贴板，请改用手动粘贴。", pixivCopyBlocked: "已转换，但浏览器阻止自动复制。请点「复制结果」按钮。" },
    "zh-TW": { clipboardEmpty: "剪貼簿文字為空。如果你是從其他 App 複製連結，請貼到手動輸入框後轉換。", clipboardImageOnly: "剪貼簿裡是圖片資料，不是連結文字。請複製連結或手動貼上文字。", clipboardNoReadableText: "剪貼簿裡沒有可讀取的文字連結。請貼到手動輸入框後轉換。", clipboardUnavailable: "目前瀏覽器/環境無法讀取剪貼簿，請改用手動貼上。", pixivCopyBlocked: "已轉換，但瀏覽器阻止自動複製。請點「複製結果」按鈕。" }
  };
  ...
  function convert(input) {
    ...
    else if ((host === "bilibili.com" || host.endsWith(".bilibili.com") || host === "b23.tv") && state.settings.siteBilibili) { siteKey = "bilibili"; siteLabel = "Bilibili"; if (host === "b23.tv") { stripParams(url, SITE_TRACKERS, cleaned); url.hostname = "vxb23.tv"; url.hash = ""; note = t("b23Note"); } else { allowOnly(url, BILI_ALLOWED, cleaned); url.hostname = "vxbilibili.com"; url.hash = ""; } }
    const convertedUrl = siteKey === "generic" ? url.toString() : url.toString().replace(/\/+$/, "");
    const output = raw === extracted ? convertedUrl : raw.replace(extracted, convertedUrl);
  ...
  function renderResult(result) { state.result = result; if (!result.ok) { toast(result.error, "warning"); return; } $("resultCard").hidden = false; $("siteBadge").textContent = result.siteLabel; $("cleanedCount").textContent = `${result.cleaned.length} ${t("cleaned")}`; $("originalUrl").textContent = result.input; $("convertedUrl").textContent = result.output; $("openBtn").href = result.convertedUrl || result.output; $("resultNote").hidden = !result.note; $("resultNote").textContent = result.note; }
  async function convertAndCopy(text, autoSave = true) { const result = convert(text); renderResult(result); if (!result.ok) return; if (result.changed) { try { await writeClipboard(result.output); toast(t("copied")); } catch (err) { toast(result.siteKey === "pixiv" ? t("pixivCopyBlocked") : `Converted, copy failed: ${err.message}`, "warning"); } if (autoSave && state.settings.saveHistory) saveHistory(result); } }
